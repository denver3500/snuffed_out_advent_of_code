# Input
input <- readLines("Day_6/input_advent.txt")
...
count <- function(tree) {
  total <- tree$depth  # This node's contribution
  for (child in tree$children) {
    total <- total + count(child)  # Add child's orbits recursively
  }
  return(total)
}
result <- count(tree)
